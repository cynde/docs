...
Jan has a default [C++ inference server (opens in a new tab)](https://github.com/janhq/nitro) built on top of [llama.cpp (opens in a new tab)](https://github.com/ggerganov/llama.cpp). This server provides an OpenAI-compatible API, queues, scaling, and additional features on top of the wide capabilities of `llama.cpp`.

## llama.cpp Extension[](#llamacpp-extension)
...
Mac IntelMac Silicon

*   Make sure you're using an Intel-based Mac. For a complete list of supported Intel CPUs, please see [here (opens in a new tab)](https://en.wikipedia.org/wiki/MacBook_Pro_\(Intel-based\)).
...
### Step 1: Open the `model.json`[](#step-1-open-the-modeljson)
...
`   {    "sources": [    {    "filename": "tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf",    "url": "https://huggingface.co/TheBloke/TinyLlama-1.1B-Chat-v1.0-GGUF/resolve/main/tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf"    }    ],    "id": "tinyllama-1.1b",    "object": "model",    "name": "TinyLlama Chat 1.1B Q4",    "version": "1.0",    "description": "TinyLlama is a tiny model with only 1.1B. It's a good model for less powerful computers.",    "format": "gguf",    "settings": {    "ctx_len": 4096,    "prompt_template": "<|system|>\n{system_message}<|user|>\n{prompt}<|assistant|>",    "llama_model_path": "tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf"    },    "parameters": {    "temperature": 0.7,    "top_p": 0.95,    "stream": true,    "max_tokens": 2048,    "stop": [],    "frequency_penalty": 0,    "presence_penalty": 0    },    "metadata": {    "author": "TinyLlama",    "tags": [    "Tiny",    "Foundation Model"    ],    "size": 669000000    },    "engine": "nitro"    }            `

### Step 2: Modify the `model.json`[](#step-2-modify-the-modeljson)
...
*   To utilize the embedding feature, include the JSON parameter `"embedding": true`. It will enable Nitro to process inferences with embedding capabilities. Please refer to the [Embedding in the Nitro documentation (opens in a new tab)](https://nitro.jan.ai/features/embed) for a more detailed explanation.
*   To utilize the continuous batching feature for boosting throughput and minimizing latency in large language model (LLM) inference, include `cont_batching: true`. For details, please refer to the [Continuous Batching in the Nitro documentation (opens in a new tab)](https://nitro.jan.ai/features/cont-batch).

If you have questions, please join our [Discord community (opens in a new tab)](https://discord.gg/Dt7MxDyNNZ) for support, updates, and discussions.

Last updated on July 25, 2024

[Local API Server](/docs/local-api "Local API Server")[TensorRT-LLM](/docs/built-in/tensorrt-llm "TensorRT-LLM")